...
import { OrderNetwork } from '../../orderNetwork.model';

@Component({
  selector: 'app-network-order',
  templateUrl: './network-order.component.html',
  styleUrls: ['./network-order.component.css'],
})
export class NetworkOrderComponent implements OnInit {
  ...
  showing: boolean;
  serviceName;
  services: any = ['أمن المعلومات والحماية التقنية'];
  servicesAr: any = ['Information security and technical protection'];
  toggling;
  ...
  onSubmitOrder() {
    ...
    console.log(orders);
    this.firestoreService.create_newOrderNetwork(orders);
    this.initOrderForm();
    this.formpop();
  }
